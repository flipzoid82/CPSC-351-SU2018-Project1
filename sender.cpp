...
void send(const char* fileName)
{
	...
	while(!feof(fp))
	{
		...
		printf("Sender says to receiver, Data is ready!\n");
		...
		printf("Receiver says, I finished saving the memory chunk\n");
	}
    ...
    /*dot operator to access the int size variable of the struct message variable sndMsg.*/
	sndMsg.mtype = SENDER_DATA_TYPE;
    sndMsg.size = 0;
    if(msgsnd(msqid, &sndMsg, sizeof(sndMsg) - sizeof(long), 0) == -1){
        perror("msgsnd");
	...
	printf("Sender says, 'I am all done, Receiver'\n");
